...
use crate::{
    ...
    value::{JsValue, PreferredType, JsVariant},
...
impl Date {
    ...
    pub(crate) fn make_date_single(
        ...
        let value = &args[0];
        let tv = if let Ok(dt) = this_time_value(value, context) { dt.0 } else {
            let tv = value.to_primitive(context, PreferredType::Default)?;
            if let JsVariant::String(ref str) = tv.variant() { match chrono::DateTime::parse_from_rfc3339(str) {
