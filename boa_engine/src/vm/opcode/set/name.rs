            ...
            if let Some(traps) = &mut context.instrumentation_conf.traps {
                ...
                if let Some(ref trap) = traps.write_trap {
                    if let Some(advice) = context.instrumentation_conf.advice() {
                        ...
                        let name = binding_locator.name().sym();

                        let js_name: JsValue =
                            context.interner().resolve_expect(name).to_string().into();
                        let result = context.call(trap, &advice, &[value, js_name]);
                        ...
                        match result {
                            Ok(value) => {
                                ...
                                if binding_locator.is_global() {
                                    if !context.realm.environments.put_value_if_global_poisoned(
                                        ...
                                        let key: JsString = context
                                            ...
                                            .into_common(false);
                                        let exists =
                                            context.global_bindings_mut().contains_key(&key);
